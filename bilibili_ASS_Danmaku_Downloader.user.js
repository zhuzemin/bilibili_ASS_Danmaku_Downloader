  ...
  getCid(function (cid) {
    ...
    fetchDanmaku(cid, function (danmaku) {
      ...
      if(!name)try { 
        ...
        let ep_item=document.querySelector('li.ep-item.cursor.badge.visited');
        let ep_title=ep_item.querySelector('span.ep-title').textContent;
        name=name+" - "+ep_title;
      }
      catch (e) { name = '' + cid; }
...
var showButton = function (count) {
  ...
  var favbar = document.querySelector('div.b-head');
...
var initButton = (function () {
  ...
  return function () {
    debug('init button');
    if (!document.querySelector('div.b-head')) return;
    getCid(function (cid) {
      debug('cid = %o', cid);
